<?php
	...
	function rotas($url) {
		...
		// ALTERAR
			...
			// Obtem dados do contato$contato escolhido para alteração
			$contato = select_where(trim($dados[1]));

			if($contato == null) {
				echo "<script> alert('Código do contato NÃO ENCONTRADO!') </script>";
			}
			else {		
				$url = "viewAlterar.php?id=".trim($dados[1])."&nome=".$contato[0]."'endereco=".$contato[1]."&telefone=".trim($contato[2]);
